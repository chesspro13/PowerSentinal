...
import { databaseTable, prepareDataInsert } from "./database_table.js";
...
function getAtResolution(data: any[], resolution: number, segments: number) {
  let cleanData: {}[] = [];
  let average = [];
  let min = 100;
  let max = 0;
  ...
  for (let i = 0; i <= segments * resolution; i += resolution) {
    for (let j = 0; j < resolution; j++) {
      if (i + j < data.length) {
        ...
        if (amperage > max) max = amperage;
        if (amperage < min) min = amperage;
      }
    }
    if (i < data.length) date = new Date(data[i]["DATETIME"]);
    else date = new Date(data[data.length - 1]["DATETIME"]);
    if (average.length != 0) {
      const load =
        Math.round(
          (average.reduce((a, b) => a + b) / average.length + Number.EPSILON) *
            100
        ) / 100;

      cleanData.push({
        time: utcToLocal(date.toISOString()),
        load: load,
        min: min,
        max: max,
      });
...
export function getLoadInRange(
  ...
  const resolution = Math.ceil(data.length / parseInt(segments));
  const cleanData = getAtResolution(data, resolution, parseInt(segments));

  return {
    METADATA: {
      DATAPOINT_COUNT: data.length,
